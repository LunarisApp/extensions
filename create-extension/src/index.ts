...
const HELP = `Create a Lunaris extension
...
export function resolveExtensionOptions(options: CliOptions): ExtensionOptions {
  ...
  if (!EXTENSION_ID_PATTERN.test(id) || id.length > MAX_EXTENSION_ID_LENGTH) {
    throw new Error(
      `Extension ID must be at most ${MAX_EXTENSION_ID_LENGTH} characters using lowercase dotted or kebab segments`
    );
  }
