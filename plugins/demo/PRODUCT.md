...
plugin developers inspecting a complete example of Lunaris view and content
type integrations.

## Product Purpose

Demo is a synthetic SaaS operations console packaged as a Lunaris plugin. It
...
clear enough to use as plugin reference material.
...
- Creating, opening, and persisting the sample dossier may use Lunaris SDK
  capabilities. The plugin makes no external requests and uses no credentials.
- The dossier is a persistent but read-only sample and compiles to portable
  structured content.

## Brand Commitments

Keep the public plugin name and ID as “Demo” and `lunaris.demo`. The interface
